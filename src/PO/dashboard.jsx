              ...
              Object.keys(allReturned[year].MLGO).forEach(mlgoUid => {
                ...
                Object.keys(mlgoReturns).forEach(assessmentId => {
                  ...
                  items.push({
                    ...
                    status: "Returned",
                   submission: returnData.submission 
  ? (typeof returnData.submission === 'string' && returnData.submission.includes('/')
      ? returnData.submission
      : new Date(returnData.submission).toLocaleDateString('en-US', {
          day: '2-digit',
          month: 'long',
          year: 'numeric'
        }))
...
useEffect(() => {
  ...
  const fetchVerifiedData = () => {
    try {
      ...
      onValue(verifiedRef, async (snapshot) => {
        if (snapshot.exists()) {
          ...
          // Iterate through years
          for (const year of Object.keys(allVerified)) {
            if (allVerified[year] && allVerified[year].LGU) {
              for (const lguKey of Object.keys(allVerified[year].LGU)) {
                const item = allVerified[year].LGU[lguKey];
                ...
                // Finally fallback to lguName
                  ...
                  municipality = lguKey.replace(`_${assessmentId}`, '').replace(/_/g, ' ');
                }
                
                // ===== IMPORTANT: Load the actual data from the reference paths =====
                let actualData = {};
                ...
                if (item.answersPath) {
                  try {
                    const answersRef = ref(db, item.answersPath);
                    ...
                    if (answersSnapshot.exists()) {
                      ...
                      // Decompress answers if needed
                      if (answers.__keyMap) {
                        // Decompress the answers
                        const decompressed = {};
                        const keyMap = answers.__keyMap;
                        Object.keys(answers).forEach(shortKey => {
                          if (shortKey !== '__keyMap') {
                            const longKey = keyMap[shortKey];
                            if (longKey) {
                              decompressed[longKey] = answers[shortKey];
                            }
                          }
                        });
                      ...
                      console.log(`✅ Loaded answers for ${municipality}:`, Object.keys(actualData).length);
                    }
                  } catch (error) {
                    console.error(`Error loading answers for ${municipality}:`, error);
                  }
                }
                
                // Load attachments from the referenced path
                if (item.attachmentsPath) {
                  try {
                    const attachmentsRef = ref(db, item.attachmentsPath);
                    ...
                    if (attachmentsSnapshot.exists()) {
                      ...
                      // Decompress attachments
                      let decompressedAttachments = compressedAttachments;
                      if (compressedAttachments.__keyMap) {
                        decompressedAttachments = {};
                        const keyMap = compressedAttachments.__keyMap;
                        Object.keys(compressedAttachments).forEach(shortKey => {
                          if (shortKey !== '__keyMap') {
                            const longKey = keyMap[shortKey];
                            if (longKey) {
                              decompressedAttachments[longKey] = compressedAttachments[shortKey];
                            }
                          }
                        });
                      }
                      
                      // Map attachments by indicator
                      ...
                      console.log(`✅ Loaded attachments for ${municipality}:`, Object.keys(attachmentsByIndicator).length);
                    }
                  } catch (error) {
                    console.error(`Error loading attachments for ${municipality}:`, error);
                  }
                }
  ...
  ? (typeof item.submission === 'string' && item.submission.includes('/') 
      ? item.submission 
      : new Date(item.submission).toLocaleDateString('en-US', {
          day: '2-digit',
          month: 'long',
          year: 'numeric'
        }))
  : new Date().toLocaleDateString('en-US', {
      ...
      year: 'numeric'
                  ...
                  data: actualData,  // ← Now contains the actual answers
                  originalData: actualData,  // ← For compatibility
                  attachmentsByIndicator: attachmentsByIndicator,  // ← Now contains actual attachments
                  type: "verified"
                });
              }
            }
          ...
          console.log("Final mapped verified data with attachments:", items);
          console.log("Items with attachments:", items.filter(i => Object.keys(i.attachmentsByIndicator).length > 0).length);
      ...
      {loadingForwarded || loadingVerified || loadingReturned ? (
          ...
          currentRows.map((item) => (
            <tr key={item.id}>
              ...
              <td>
  {item.submission && item.submission !== "N/A" && item.submission !== "-" 
    ? (() => {
        try {
